...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'shinobi', 'Shinobi', and 'SHINOBI'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.shinobi/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.shinobi/g),
   ...
   * Rename files.
   *
   * Rename files that had shinobi in their name, such as
   * css/shinobi.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*shinobi*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('shinobi', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-shinobi-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob Shinobi's files so that we can process them.
    glob('*', { ignore: 'node_modules/**' }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace shinobi name mentions in files.
      generator.replaceFileContents();
      // Rename files that had shinobi in their name.
